    ...
    if (prop === "resource") {
      ...
      return (name: string, pattern: any, handler: any) => {
        try {
          ...
          const contextAwareHandler = async (
            uri: any,
            params: any,
            extra: any
          ) => {
...
logger.info(
  `Atlassian config available: ${JSON.stringify(atlassianConfig, null, 2)}`
);
  ...
  tool: (name: string, description: string, schema: any, handler: any) => {
    ...
    server.tool(
      ...
      async (params: any, context: any) => {
        ...
        logger.debug(
          `Tool ${name} called with context keys: [${Object.keys(context)}]`
        );
...
async function startServer() {
  try {
    ...
    logger.info(
      "- Jira filter tools: createFilter, updateFilter, deleteFilter"
    );
    logger.info(
      "- Jira sprint tools: createSprint, startSprint, closeSprint, addIssueToSprint"
    );
    ...
    logger.info(
      "- Jira dashboard tools: createDashboard, updateDashboard, addGadgetToDashboard, removeGadgetFromDashboard"
    );
    logger.info(
      "- Confluence tools: createPage, updatePage, addComment, addLabelsToPage, removeLabelsFromPage"
    );
